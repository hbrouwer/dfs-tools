...
%>> forall(x,imp(person(x),forall(y,imp(and(place(y),enter(x,y)),forall(z,imp(and(place(z),enter(x,z)),z=y)))))).
...
%>> forall(x,imp(person(x),forall(y,imp(food(y),forall(z,imp(and(food(z),and(order(x,y),order(x,z))),z=y)))))).
